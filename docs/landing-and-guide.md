...
바꿔치기를 `site.js`에만 맡기면 늦는다. `site.js`는 `defer`라서 문서 파싱이 끝난 뒤에 도는데, 프리로드 스캐너는 그보다 훨씬 먼저 파서를 앞질러 달리며 eager `<img src>`를 전부 받아온다. 직전 커밋을 그대로 띄워 실측한 결과, 영어 방문자는 한국어 이미지 **여섯 장(449,159 B)** 을 받아놓고 버린 뒤 영어본을 다시 받았다.
...
`loading="lazy"`는 프리로드 스캐너의 선행 출발을 포기하는 대가를 치른다. 이 페이지는 i18n 엔진과 사전 네 개가 `<head>`를 막고 있어 그 차이가 작지 않다. 해결된 언어로 `<link rel="preload" as="image">`를 주입해 그 출발을 되찾는 방법을 넣어 보고 실측했는데, 결과가 더 나빴다 — 프리로드와 이미지 자신의 로드가 하나로 합쳐지지 않아서 한국어 방문자가 히어로를 **두 번**(당시 103,470 B짜리 이미지를 206,940 B) 받았고 영어 방문자도 열 번 중 네 번쯤 그랬다. 이미지 한 장을 통째로 두 번 받는 비용이 발견 시점을 앞당겨 버는 것보다 크므로 프리로드는 넣지 않는다. 지금은 방문자당 정확히 한 번, 자기 언어로만 받는다(영어 101,744 B / 한국어 101,140 B, 실측). 다시 시도한다면 그림이 제대로 나오는지가 아니라 `requestfinished` 횟수와 로드당 전송 바이트를 재야 한다.
