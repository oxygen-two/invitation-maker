...
- The design gallery becomes a six-up "shelf" with alternating card heights at desktop, three-per-row at tablet, and a horizontal snap strip with a right-edge fade hint on phones. The landing page's design shelf sat outside the studio UI/UX audit's scope (that audit covered the studio, the shared viewer, and the error pages, not `index.html`), so this has no B-item of its own; the closest precedent is B-10's fade-masked occasion-chip row, whose "there's more, keep scrolling" pattern this reuses for the design shelf.
...
## Global readiness — 2026-09-18

- Studio chrome token layer: `assets/studio/studio.css` declares 21 `--studio-*` custom properties on `:root` — surfaces (`--studio-paper`, `--studio-surface`, `--studio-surface-alt`, `--studio-surface-sunken`), ink (`--studio-ink`, `--studio-ink-muted`, `--studio-ink-soft`), the one accent (`--studio-accent`, `--studio-accent-deep`, `--studio-accent-ink`), lines (`--studio-line`, `--studio-line-strong`, `--studio-control-line`), focus (`--studio-focus`, `--studio-focus-ring`), the "this only lives in this browser" warning (`--studio-warn-bg`, `--studio-warn-line`, `--studio-warn-ink`), `--studio-danger` for destructive actions and nothing else, and two composited values built from the ink channel triple — `--studio-scrim` and `--studio-shadow` — for dialog backdrops and elevation. Every editor chrome component — item card meta, add-item buttons, panel eyebrows, upload labels, the hero image picker — is pinned to these tokens, never to the applied invitation's own palette variables, which now exist only inside the preview iframe's document. `html { background: var(--studio-paper) }` closes the bug where an invitation's palette painted the page background and overscroll bands.
- Mobile sample sheet: tapping a gallery card below 900px opens `#sample-sheet` (`.studio-sheet`, a `<dialog>`), a bottom sheet rendered through the same `InvitationCore.buildStandaloneHtml` path as the preview iframe, so a phone user sees the design at real size before applying it instead of a thumbnail alone. "Use this design" inside the sheet calls the existing apply handler and dismisses the sheet.
- Item cards: the header collapsed to a menu (`.content-item-menu`, ⋯ button) holding Move up / Move down / Delete, replacing a row of buttons that wrapped onto a second line on narrow cards. Delete no longer opens `window.confirm`; it swaps in an inline confirm row inside the same card.
- Single gallery CTA: the gallery's apply row is the one place that names the selected design and applies it. The former second "Apply this design" notice under the preview panel is gone; nothing else states the same fact twice.
- Library empty state: an inline SVG envelope, `library.emptyTitle` / `library.emptyBody` copy, and a `Start a new invitation` button that jumps to the gallery stage replace the bare "Nothing here yet." text.
- Phone header short status: `#draft-status` collapses to `status.draftSavedShort` ("Saved") below 900px instead of the full sentence wrapping to two lines; the full sentence stays for screen readers via `#draft-status-text`.
- Share dialog: one heading (`finish.shareTitle`) — the former nested "SHARE / Public link" heading is gone. The expiry sentence (`finish.shareDialogPrivacy`, linking to `/privacy`) renders before the publishing panel, so the retention rule is visible before a guest publishes, not after. Once published, the panel offers the link, Copy, a dependency-free QR code (`assets/publishing/qr.js`, byte mode, EC level M), the system share sheet where `navigator.share` exists, and a "Copy invitation message" button.
- Date, time zone and calendar file: `studio.html` offers a real `datetime-local` input plus a time-zone select, with the old free-text `dateLabel` field collapsed behind a "Write it my own way" toggle for anyone who still wants full control of the wording. A generated date renders per the reader's own locale (`InvitationI18n.formatSampleDate`, `Intl`-backed, including `en-GB`/`en-AU` variants); a typed `dateLabel` still wins verbatim, unchanged. `InvitationCore.renderCalendarLink` adds an "Add to calendar" `.ics` download next to the date whenever `dateTime` is set.
- Dark mode: `shared.html`, `viewer.html`, `index.html`, `guide.html` and `assets/site/**` define light tokens on `:root` and override them under `prefers-color-scheme: dark` and `[data-theme="dark"]`, with a matching dark `theme-color` meta. The studio editor, the invitation renderers, and the error pages (which carry their own separate dark palette, see "Error pages" above) are deliberately untouched by this pass.
- Error pages speak the guest's language: both Korean and English copy travel inside every generated page as inline JSON (`errorPages.<status>.*` in the dictionaries), resolved client-side by the same rules as `assets/i18n/i18n.js` (`?lang`, stored choice, `navigator.languages`, default `ko`) with no network request. A visitor with JavaScript disabled still gets the Korean the markup was generated with.
- Occasion groups: every occasion carries a `group` (`celebrate` | `milestone` | `family` | `gather`); the gallery's chip rows print each group that actually has occasions, in that fixed order, with a group label above its chips. Three new occasions — baby shower (`family`), graduation (`milestone`), housewarming (`gather`) — each ship two template designs reusing existing visual families rather than inventing new ones.
- Consent banner: `assets/site/consent.js` shows an Accept / Essential-only banner (both controls 44px) until a choice is stored; product analytics (GA4, PostHog, Vercel Analytics) stay inert until the visitor accepts, while `client_error` diagnostics remain exempt as documented in `docs/analytics.md`.

## Studio accessibility conventions — 2026-09-23

- **One persistent heading.** `#studio-heading` is the studio's only `<h1>`, a sibling of `.app-shell` that no per-stage CSS rule can hide. It renders the active stage's own name from the same dictionary keys the stage nav uses and takes focus (`tabindex="-1"`, so only programmatically) on every stage change, so assistive tech always lands on a heading that matches what's now on screen. `#studio-stage-status` (`role="status"`) announces the same change politely, for a screen reader that reports a live region sooner than it reports a focus move.
- **A skip link on every page that ships one.** `.skip` is the first focusable element on the page, landing on `#main`. The consent banner deliberately inserts itself just after the skip link rather than at the top, so reaching the content is still one Tab and answering the banner is two.
- **Confirm in the page, never `window.confirm`.** Every destructive or hard-to-reverse action — deleting an item, deleting a library entry, revoking a publication, or publishing with an unreachable reply-contact — asks through a native `<dialog>` (`#confirm-dialog`, or an item card's own inline confirm row): a stated question, an accept button, and a cancel path (✕, Escape, or the backdrop) that all resolve to "no." A dialog that cannot be shown answers no by default — not being able to ask is not permission to go ahead. `window.confirm` cannot be translated or styled and, as of PR #56, is no longer used anywhere in the studio.
- **A disclosure menu, not a button row, for secondary per-item actions.** An item card's header collapses "move up / move down / delete" into one `⋯` menu (`.content-item-menu`) instead of a row of buttons that used to wrap onto a second line on narrow cards.
