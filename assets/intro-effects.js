(function (root) {
  ...
  const petalPositions = Object.freeze([
    [8, 18, 0, .9], [24, 36, .18, 1.15], [42, 14, .08, .8], [61, 43, .28, 1.05], [82, 22, .12, .95],
  ...
  const renderDecor = (effect, invitation) => {
    ...
    if (effect === "petals") {
      return `<div class="intro-petals" aria-hidden="true">${petalPositions.map(([x, y, delay, scale]) => `<span style="--petal-x:${x}%;--petal-y:${y}%;--petal-delay:${delay}s;--petal-scale:${scale}"></span>`).join("")}</div>`;
    }
    if (effect === "spotlight") {
...
.intro-skip{position:absolute;z-index:5;top:max(16px,env(safe-area-inset-top));right:max(16px,env(safe-area-inset-right));min-height:40px;padding:0 14px;border:1px solid rgba(42,23,32,.18);border-radius:999px;background:rgba(255,250,242,.86);color:var(--ink,#2a1720);font:700 13px/1 var(--font-ko,"Noto Sans KR",sans-serif);cursor:pointer}
    ...
    .intro-envelope{display:grid;place-items:center}.intro-envelope-card,.intro-envelope-pocket,.intro-envelope-flap,.intro-envelope-seal{position:absolute;width:min(78vw,390px);aspect-ratio:1.48/1;left:50%;bottom:13%;border:1px solid rgba(122,36,59,.22);box-shadow:0 20px 58px rgba(45,11,22,.18);transform:translateX(-50%)}.intro-envelope-card{z-index:1;width:min(66vw,330px);bottom:27%;border-radius:2px;background:linear-gradient(145deg,rgba(255,255,255,.98),var(--paper,var(--cream-50,#fffaf2)));animation:intro-envelope-card 3.2s cubic-bezier(.22,1,.36,1) forwards}.intro-envelope-pocket{z-index:3;background:var(--paper,var(--cream-50,#fffaf2));clip-path:polygon(0 0,50% 49%,100% 0,100% 100%,0 100%)}.intro-envelope-flap{z-index:4;transform:translateX(-50%);transform-origin:50% 0;background:linear-gradient(145deg,var(--paper,var(--cream-50,#fffaf2)),rgba(246,221,166,.84));clip-path:polygon(0 0,100% 0,50% 58%);animation:intro-envelope-flap 3.2s cubic-bezier(.22,1,.36,1) forwards}.intro-envelope-seal{z-index:5;width:42px;aspect-ratio:1;bottom:calc(13% + min(24vw,120px));border-radius:50%;background:var(--gold,var(--gold-500,#d9ac54));box-shadow:0 4px 14px rgba(45,11,22,.18);animation:intro-envelope-seal 3.2s ease forwards}
    .intro-card-plane{background:linear-gradient(180deg,var(--deep,var(--wine-900,#42101f)),var(--mid,var(--wine-700,#7a243b)));transform-origin:var(--intro-target-origin,center);animation:intro-card-shrink 3s ease forwards}
.intro-dawn{background:radial-gradient(circle at 50% 32%,rgba(255,245,210,.9),transparent 34%),linear-gradient(180deg,#151015,var(--bg,var(--page-bg-a,#ead5ce)));animation:intro-dawn 2.6s ease forwards}
.intro-curtain{display:grid;grid-template-columns:1fr 1fr}.intro-curtain span{background:linear-gradient(90deg,var(--deep,var(--wine-900,#42101f)),var(--mid,var(--wine-700,#7a243b)));animation:intro-curtain-left 3s ease forwards}.intro-curtain span+span{animation-name:intro-curtain-right}
    .intro-bursts{background:radial-gradient(circle at 50% 48%,rgba(91,45,56,.38),transparent 42%),linear-gradient(160deg,#100d14,#21121c 54%,#0c0d13)}.intro-burst{position:absolute;left:var(--origin-x);top:var(--origin-y);width:5px;height:5px;border-radius:50%;color:var(--tone);background:currentColor;box-shadow:0 0 16px currentColor;animation:intro-burst-glow 1.9s ease var(--burst-delay) forwards}.intro-burst span{position:absolute;left:1px;top:1px;display:block;width:4px;height:4px;border-radius:50%;background:currentColor;box-shadow:0 0 13px currentColor;transform:rotate(var(--angle)) translateY(0);animation:intro-firework 1.8s cubic-bezier(.16,.84,.32,1) calc(var(--burst-delay) + var(--spark-delay)) forwards}.intro-petals span{position:absolute;left:var(--petal-x);top:var(--petal-y);display:block;width:10px;height:10px;border:1px solid var(--particle-edge,rgba(66,16,31,.48));border-radius:70% 0 70% 0;background:var(--particle-accent,var(--mid,#7a243b));filter:drop-shadow(0 2px 3px rgba(42,23,32,.18));opacity:.9;transform:scale(var(--petal-scale));animation:intro-float 3.2s ease var(--petal-delay) forwards}.intro-petals span:nth-child(3n){background:var(--particle-light,var(--gold,#d9ac54))}.intro-petals span:nth-child(4n){background:var(--particle-alt,var(--soft,#65535a))}.invitation-intro[data-intro-effect="fireworks"] .intro-copy h1{color:#fffdf9;text-shadow:0 2px 18px rgba(0,0,0,.48)}.invitation-intro[data-intro-effect="fireworks"] .intro-subtitle,.invitation-intro[data-intro-effect="fireworks"] .intro-date,.invitation-intro[data-intro-effect="fireworks"] .intro-host{color:rgba(255,250,242,.88)}.invitation-intro[data-intro-effect="fireworks"] .intro-skip{border-color:rgba(255,250,242,.32);background:rgba(16,13,20,.7);color:#fffdf9}
...
.intro-photo,.intro-photo-fallback{display:grid;place-items:center;background:linear-gradient(145deg,var(--bg,var(--page-bg-a,#ead5ce)),var(--paper,var(--cream-50,#fffaf2)))}.intro-photo img{width:100%;height:100%;object-fit:cover;filter:blur(16px);transform:scale(1.08);animation:intro-photo-focus 3.6s ease forwards}.intro-photo-fallback{background:radial-gradient(circle at 50% 38%,rgba(217,172,84,.35),transparent 30%),linear-gradient(160deg,var(--paper,var(--cream-50,#fffaf2)),var(--bg,var(--page-bg-a,#ead5ce)))}
    .is-intro-active .particle-layer,.is-intro-active .particle-layer span,.is-intro-active .particle-layer span::before{animation-play-state:paused}
