...
const { DAY_MS, expiryTargetAt } = require("../server/publishing/expiry.cjs");
...
/* The expiry a record would get if it were published under today's policy —
   sliding window, hard ceiling, and the event floor that outranks both. That
   formula lives in server/publishing/expiry.cjs and is the one the API writes
   with; this script used to keep a second copy of it, which is why PR #47's
   event floor never reached the backfill. A migration that disagrees with the
   running service about when a link dies is worse than no migration.

   Grace decision: a record whose honest expiry is already in the past would
   stop being readable the moment this wrote it. Killing a live invitation on
   the spot is unacceptable collateral, so those get `now + idleWindow`
   instead: anyone sharing that link today still has a full idle window, and
   normal reads take over from there (a link nobody opens then expires on
   schedule). */
const createExpiryPlanner = ({ idleWindowDays, maxLifetimeDays, eventGraceDays, maxEventLeadDays }) => {
  const idleWindowMs = Math.max(0, idleWindowDays) * DAY_MS;
  const graceExpiry = (now) => new Date(now.getTime() + idleWindowMs);

  return (record, now) => {
    if (!asDate(record.createdAt)) {
      return { bucket: "missingCreatedAt", expiresAt: graceExpiry(now) };
    }
    ...
    if (!target || target.getTime() <= now.getTime()) {
      return { bucket: "grace", expiresAt: graceExpiry(now) };
    }
    return { bucket: "normal", expiresAt: target };
  };
};

const run = async ({ apply, collectionName, mongoUri, mongoDbName, parsedUri, plan, publishing }) => {
  const { idleWindowDays, maxLifetimeDays } = publishing;
  ...
  const cursor = collection.find({}, {
    // The event floor reads the invitation's own date, so the projection
    // carries it: without it every record would look like one with no event.
    projection: { id: 1, createdAt: 1, expiresAt: 1, expiresAtDate: 1, "invitation.dateTime": 1 }
  });
  ...
  console.log(`  policy: idle window ${idleWindowDays}d, max lifetime ${maxLifetimeDays}d, event grace ${publishing.eventGraceDays}d, reference time ${now.toISOString()}`);
  ...
  console.log(`    grace bucket:     ${summary.grace} (already past their expiry; would stop being readable immediately, given now + ${idleWindowDays}d instead)`);
...
const main = () => {
  ...
  const publishing = readPublishingConfigFromEnv();
  if (!(publishing.maxLifetimeDays > 0)) {
  ...
  return run({
    apply,
    collectionName: process.env.PUBLISH_COLLECTION || "published_invitations",
    mongoDbName,
    mongoUri,
    parsedUri,
    plan: createExpiryPlanner(publishing),
    publishing
  });
};
...
module.exports = { createExpiryPlanner };
